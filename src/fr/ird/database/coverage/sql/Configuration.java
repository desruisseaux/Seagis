...
{       
     ...
     */
    public static final Key KEY_SERIES_TREE        = Key.get(Table.SERIES+":TREE", 
                                                             ResourceKeys.SQL_SERIES_TREE, 
                                                             "SELECT subseries.id, subseries.name, subseries.description, series.id, series.name, series.description, operations.id, operations.name, operations.description, parameters.ID, parameters.name, parameters.description, subseries.format, series.period FROM \"" + Table.PARAMETERS + "\" parameters, \"" + Table.OPERATIONS + "\" operations, \"" + Table.SERIES+ "\" series, \"" + Table.SUBSERIES + "\" subseries WHERE parameters.id=parameter AND operations.id=operation AND series.id=series AND visible=TRUE ORDER BY parameters.name, operations.name, series.name"),
                                                             
                            KEY_SERIES_ID          = Key.get(Table.SERIES+":ID", 
    ...
    {
        super(CoverageDataBase.getDefaultFileOfConfiguration());    
    }    
