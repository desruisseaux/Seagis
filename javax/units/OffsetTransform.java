...
final class OffsetTransform extends UnitTransform
{
    ...
    private OffsetTransform(final Unit fromUnit, final Unit toUnit, final double offset) throws UnitException
    {
        ...
        if (Double.isNaN(offset) || Double.isInfinite(offset))
            throw new UnitException(Resources.format(ResourceKeys.NOT_A_NUMBER_$1, new Double(offset)));
    }
    ...
    {
        if (offset==0) return IdentityTransform.getInstance(fromUnit, toUnit);
        else return new OffsetTransform(fromUnit, toUnit, offset).intern();
    }
