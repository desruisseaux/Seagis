...
import javax.units.resources.Resources;
import javax.units.resources.ResourceKeys;


/**
 * Classe représentant des unités décalées par rapport à d'autres unités.
 * Les valeurs exprimées selon les unités d'un objet <code>OffsetUnit</code> peuvent être
 * converties vers le système d'unités {@link #unit} à l'aide de l'équation suivante:
 *
 * <blockquote><pre>
 ...
 * différentes unités, par exemple des degrés Kelvin aux degrés Celsius. Cette classe n'ayant pas
 * de constructeur publique, la seule façon d'obtenir des unités décalées est d'utiliser les méthodes
 * {@link #getInstance} ou {@link #offset}.
 *
...
/*public*/ final class OffsetUnit extends Unit
{
    ...
    private OffsetUnit(final double offset, final Unit unit, final String symbol, final PrefixSet prefix)
    {
        ...
        if (unit==null)
            throw new NullPointerException(Resources.format(ResourceKeys.NO_UNIT));
        if (Double.isNaN(offset) || Double.isInfinite(offset))
            throw new IllegalArgumentException(Resources.format(ResourceKeys.NOT_A_NUMBER_$1, new Double(offset)));
    }
    ...
    {
        if (fromUnit==this) return value; // sligh optimization
    ...
    public UnitTransform getTransform(final Unit fromUnit) throws UnitException
    {
        ...
        else return IdentityTransform.getInstance(fromUnit, this);
    }
    ...
    {
        if (toUnit==this) return value; // sligh optimization
    ...
    protected void inverseConvert(final double[] values, final Unit toUnit) throws UnitException
    {
        if (!equalsIgnoreSymbol(toUnit))
        {
            ...
            else throw toUnit.incompatibleUnitsException(this);
        }
    }
    ...
    protected UnitTransform getInverseTransform(final Unit toUnit) throws UnitException
    {
        ...
        else return IdentityTransform.getInstance(this, toUnit);
    }
    ...
    /**
     * Après la lecture d'une unité, vérifie si cette unité
     * apparaît déjà dans la banque des unités {@link #pool}.
     * Si oui, l'exemplaire de la banque sera retourné plutôt
     * que de garder inutilement l'unité courante comme copie.
     */
