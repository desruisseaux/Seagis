...
final class ScaledTransform extends UnitTransform
{
    ...
    private ScaledTransform(final Unit fromUnit, final Unit toUnit, final double amount) throws UnitException
    {
        ...
        if (Double.isNaN(amount) || Double.isInfinite(amount) || amount==0)
            throw new UnitException(Resources.format(ResourceKeys.NOT_DIFFERENT_THAN_ZERO_$1, new Double(amount)));
    }
    ...
    {
        if (amount==1) return IdentityTransform.getInstance(fromUnit, toUnit);
        else return new ScaledTransform(fromUnit, toUnit, amount).intern();
    }
