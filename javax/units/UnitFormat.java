...
final class UnitFormat
{
    ...
    final StringBuffer format(final Factor[] factors, StringBuffer buffer)
    {
        ...
        for (int i=0; i<factors.length; i++)
        {
            ...
            else if (factor.power<0) denominatorCount++;
        }
        /*
        ...
        if (denominatorCount!=0)
        {
            ...
            if (denominatorCount!=1) buffer.append(CLOSE_SYMBOL);
        }
        return buffer;
    }
    ...
    private String parse(String symbol, final Set set) throws IllegalArgumentException
    {
        ...
        if (true)
        {
            ...
            if (level!=0)
            {
                throw new IllegalArgumentException(Resources.format(ResourceKeys.NON_EQUILIBRATED_PARENTHESIS_$2, symbol, String.valueOf(level>=0 ? CLOSE_SYMBOL : OPEN_SYMBOL)));
            }
            if (index==length)
            {
                upper=length-upper;
