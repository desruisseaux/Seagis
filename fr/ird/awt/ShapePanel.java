...
/**
 * Afficheur de formes géométriques. Cette classe est réservée au déboguage d'objets
 * {@link Shape}. Elle peut faire apparaître dans une fenêtre une ou plusieurs formes
 * géométriques et y superposer des cercles de couleurs différentes selon qu'ils se
 * trouvent à l'intérieur ou à l'extérieur de la forme géométrique. L'origine (0,0)
...
{
    /**
     * Interface des objets capables de produire
     * des formes géométriques. Ces formes sont
     * généralement calculées au hasard à des
     * fins de test.
     */
    ...
    private void packShapes()
    {
        ...
        for (int i=0; i<shapes.length; i++)
        {
            ...
            if (object instanceof Point2D)
            {
                ...
                if (bounds!=null) bounds.add(point);
                else bounds=new Rectangle2D.Double(point.getX(), point.getY(), 0, 0);
            }
    ...
    private boolean intersects(final Rectangle2D bounds)
    {
        for (int i=0; i<shapes.length; i++)
        {
            ...
            if (object instanceof Point2D)
            {
                if (bounds.contains((Point2D) object)) return true;
                continue;
            }
            if (object instanceof Shape)
            {
                if (((Shape) object).intersects(bounds)) return true;
                continue;
            }
        }
        return false;
    ...
    private JMenuBar createMenuBar()
    {
         ...
         * Menu AFFICHAGE
         */
        menu=menubar.add(new JMenu("Affichage"));
        /*
        ...
        if (true)
        {
            ...
            final String[] labels=new String[]
            {
                ...
                "Rectangles à l'intérieur",
                "Rectangles interceptant",
                "Aucun"
            };
        ...
        if (true)
        {
            final JCheckBoxMenuItem item=new JCheckBoxMenuItem("Points de contrôles", controlPointVisible);
            item.addActionListener(new ActionListener()
            {
                public void actionPerformed(final ActionEvent event)
                {
        ...
        if (true)
        {
            final JCheckBoxMenuItem item=new JCheckBoxMenuItem("Boîtes cadres", boundsVisible);
            item.addActionListener(new ActionListener()
            {
                public void actionPerformed(final ActionEvent event)
                {
        ...
        if (true)
        {
            final JCheckBoxMenuItem item=new JCheckBoxMenuItem("Pleine fenêtre", packShapes);
            item.addActionListener(new ActionListener()
            {
                public void actionPerformed(final ActionEvent event)
                {
        ...
        if (true)
        {
            final JMenuItem item=new JMenuItem("Nouveau");
            menubar.add(item);
    ...
    private Shape toShape(final Point2D point)
    {
        if (circle==null)
        {
            return circle=new Ellipse2D.Double(point.getX()-CIRCLE_SIZE/2, point.getY()-CIRCLE_SIZE/2, CIRCLE_SIZE, CIRCLE_SIZE);
        }
        else
        {
                        ...
                        if (shapes[i] instanceof Shape)
                        {
                            ...
                            switch (test)
                            {
                                ...
                                case TEST_CONTAINS_SHAPE:
                                {
                                    ...
                                    draw=bounds; if (shape.contains(bounds))
                                    {
                                        inside=true;
                                        break check;
                                    }
                                ...
                                {
                                    final Rectangle2D bounds=draw.getBounds2D();
                                    draw=bounds; if (shape.intersects(bounds))
                                    {
                                        inside=true;
                                        break check;
                                    }
     ...
     */
    public static Point2D[] getPoints(final int count, final String[] args) throws IllegalArgumentException
     ...
     * @throws IllegalArgumentException si <code>args</code> n'a pas la longueur requise.
     */
    private static Point2D[] getPoints(final int count, final String[] args, int i) throws IllegalArgumentException
    {
        ...
        {
            throw new IllegalArgumentException("Il faut "+count+" points.");
        }
        return points;
    }
     ...
     * correctement.
     */
    public static void show(final Object[] shapes)
    {show(new ShapePanel(shapes));}
     ...
     * correctement.
     */
    public static void show(final Producer producer)
    {show(new ShapePanel(producer));}
     ...
     */
    public static void main(final String[] args) throws ClassNotFoundException, IllegalAccessException, InstantiationException, ClassCastException
    {
