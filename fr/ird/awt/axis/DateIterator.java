...
{
    /**
     * Nombre de millisecondes dans
     * certaines unités de temps.
     */
    ...
    /**
     * Format à utiliser pour écrire les étiquettes de
     * graduation.  Ce format ne sera construit que la
     * première fois où {@link #getLabel} sera appelée.
     */
    private transient DateFormat format;

    /**
     * Code du format utilisé pour construire le champ de date de {@link #format}.  Les codes valides sont
     * notamment {@link DateFormat#SHORT}, {@link DateFormat#MEDIUM} ou {@link DateFormat#LONG}. La valeur
     * -1 indique que le format ne contient pas de champ de date, seulement un champ des heures.
     */
    private transient int dateFormat = -1;

    /**
     * Code du format utilisé pour construire le champ des heures de {@link #format}. Les codes valides sont
     * notamment {@link DateFormat#SHORT}, {@link DateFormat#MEDIUM} ou {@link DateFormat#LONG}.   La valeur
     * -1 indique que le format ne contient pas de champ des heures, seulement un champ de date.
     */
     ...
     */
    protected void init(final long minimum, final long maximum, final float visualLength, final float visualTickSpacing)
    {
        ...
        this.increment         = Math.round((maximum-minimum)*((double)visualTickSpacing/(double)visualLength));
        /*
        ...
        if (subTickField==Calendar.MILLISECOND) subTickAdd*=10;
        findFirstTick();
    }
    ...
    {
        int factor=1;
        while (factor<=increment) factor*=10;
    ...
    /**
     * Indique s'il reste des graduations à retourner. Cette méthode
     * retourne <code>true</code> tant que {@link #getValue} ou
     * {@link #getLabel} peuvent être appelées.
     */
    ...
    public String getLabel()
    {
        if (!formatValid)
        {
            ...
            if (dateFormat!=this.dateFormat || timeFormat!=this.timeFormat || format==null)
            {
                ...
                if      (dateFormat == -1)
                    if  (timeFormat == -1) format=DateFormat.getDateInstance    (DateFormat.DEFAULT,     locale);
                    else                   format=DateFormat.getTimeInstance    (            timeFormat, locale);
                else if (timeFormat == -1) format=DateFormat.getDateInstance    (dateFormat,             locale);
                else                       format=DateFormat.getDateTimeInstance(dateFormat, timeFormat, locale);
    ...
    /**
     * Passe directement à la graduation
     * majeure suivante.
     */
    ...
    /**
     * Replace l'itérateur sur
     * la première graduation.
     */
    public void rewind()
    {
        this.value       = value0;
