...
class NumberIterator implements TickIterator
{
     ...
     */
    protected final void init(double minimum, final double maximum, final float visualLength, final float visualTickSpacing)
    {
    ...
    public String getLabel()
    {
        if (!formatValid)
        {
            ...
            /*
             * Trouve le nombre de chiffres après la virgule nécessaires pour représenter les étiquettes de la
             * graduation. Impose une limite de six chiffres, limite qui pourrait être atteinte notamment avec
             * les nombres périodiques (par exemple des intervalles de temps exprimés en fractions de jours).
             */
            int precision;
            ...
            for (precision=0; precision<6; precision++)
            {
                ...
                if (!(check > step*EPS)) break; // 'step' may be NaN
                step *= 10;
            }
