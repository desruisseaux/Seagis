...
public class SQLEditor extends JPanel
{
    ...
    private final class Model extends AbstractListModel implements ListSelectionListener, ActionListener
    {
        ...
        public void valueChanged(final ListSelectionEvent event)
        {
            ...
            {
                // Sauvegarde la requête SQL que l'utilisateur
                // vient de modifier. Cette modification n'est
                // pas encore transmise à l'objet Preferences.
                // Cette étape sera faite à la fin par la méthode
                // save() si l'utilisateur clique sur "Ok"
                final String[] record=toDisplay.get(index);
                record[VALUE]=valueArea.getText();
            }
            valueTextChanged();
        }
    ...
    public boolean showDialog(final Component owner)
    {
        ...
        toDisplay.get(model.index)[VALUE] = valueArea.getText();
        if (ok) save();
        return ok;
    }
