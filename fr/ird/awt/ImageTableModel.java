 ...
 * le pont entre une table {@link ImageTable} et l'afficheur {@link JTable} de
 * <i>Swing</i>. Les données d'une table d'images peuvent être affichées comme
 * suit:
 *
 * <blockquote><pre>
 * final {@link ImageTable} table = ...;
 * final {@link TableModel} model = new ImageTableModel(table);
 ...
 * </pre></blockquote>
 *
 * La classe <code>ImageTableModel</code>  garde une trace des images qui sont
 * ajoutées ou retirées de la table. Ces opérations peuvent être annulées. Les
 * fonctions "annuler" et "refaire" peuvent être activées avec le code suivant:
 *
 * <blockquote><pre>
 * final {@link UndoManager} undoManager=new UndoManager();
 * ((ImageTableModel) model).addUndoableEditListener(undoManager);
 * </pre></blockquote>
 *
...
public class ImageTableModel extends AbstractTableModel
{
    ...
    /**
     * Construit une table qui contiendra la liste des images de <code>table</code>. Ce constructeur
     * mémorise la série de <code>table</code>, telle que retournée par {@link ImageTable#getSeries}.
     *
     ...
     * tandis que les clés seront les objets {@link ImageEntry} qu'ils
     ...
     * retournés par {@link ImageEntry#getID}.  Cette méthode peut
     ...
     * {@link ImageEntry#getID}). Cette méthode est l'inverse de
     * {@link #getEntryIDs}.
     *
...
//      remove(Collections.singleton(unwrap(toRemove)));
    ...
    /**
     * Classe pour afficher des cellules de {@link ImageTableModel} dans une table
