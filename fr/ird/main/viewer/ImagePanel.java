...
final class ImagePanel extends JPanel
{
    ...
    /**
     * Retourne le paneau {@link MosaicPanel} dans lequel est insérée
     * cette image, ou <code>null</code> s'il n'y en a pas.
     */
    private MosaicPanel getMosaic()
    {
        for (Container parent=getParent(); parent!=null; parent=parent.getParent())
            if (parent instanceof MosaicPanel) return (MosaicPanel)parent;
        return null;
    }
    ...
    private void setImage(final Layer[] visualLayers, final ImageEntry entry, final GridCoverage image) // NO synchronize here!
    {
        ...
        synchronized (getTreeLock())
        {
             ...
             */
            final MosaicPanel mosaic = getMosaic();
    ...
    private final class Reader extends Thread implements IIOReadWarningListener
    {
            ...
            while (!kill)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    synchronized (this)
                    {
                        if (entry.equals(this.entry))
                        {
                            ExceptionMonitor.show(ImagePanel.this, exception);
                        }
                    }
                }
                catch (OutOfMemoryError error)
                {
                    ...
                    System.gc();
                    ExceptionMonitor.show(ImagePanel.this, error);
                }
                finally
                {
                    /*
        ...
        /**
         * Retourne la fenêtre interne dans laquelle apparaît l'objet {@link ImagePanel}.
