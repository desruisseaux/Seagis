...
final class ImageTableImpl extends Table implements ImageTable
{
    ...
    public synchronized void setSeries(final SeriesEntry series) throws SQLException
    {
        if (!series.equals(this.series))
        {
            ...
            log("setSeries", Clé.SET_SERIES¤1, series.getName());
        }
    }
    ...
    public synchronized void setTimeRange(final Date startTime, final Date endTime) throws SQLException
    {
        ...
        if (newStartTime!=this.startTime || newEndTime!=this.endTime)
        {
            ...
            log("setTimeRange", Clé.SET_TIME_RANGE¤2, new String[]{startText, endText});
        }
    }
    ...
    public synchronized void setGeographicArea(final Rectangle2D rect) throws SQLException
    {
        if (!rect.equals(geographicArea))
        {
            ...
            log("setGeographicArea", Clé.SET_GEOGRAPHIC_AREA¤1, getStringArea());
        }
    }
    ...
    public synchronized void setPreferredResolution(final Dimension2D pixelSize)
    {
        if (!XClass.equals(resolution, pixelSize))
        {
            ...
            log("setPreferredResolution", clé, param);
        }
    }
     ...
     * cette classe.
     *
     * @return Tableau d'images qui interceptent la plage de temps et la région géographique d'intérêt.
     * @throws SQLException si une erreur est survenu lors de l'accès à la base de données.
     */
  ...
  loop: while (result.next())
        ...
        log("getEntries", Clé.FOUND_IMAGES¤1, new Integer(entries.length));
        return entries;
    }
    ...
    public ImageEntry getEntry() throws SQLException
    {
        ...
        final ImageEntry[] entries=getEntries();
        if (entries!=null)
        {
            final ImageComparator comparator=new ImageComparator(this);
            for (int i=0; i<entries.length; i++)
                if (best==null || comparator.compare(entries[i], best)<=-1)
                    best = entries[i];
        }
        return best;
    }
     ...
     */
    private static void log(final String method, final int clé, final Object param)
    {
        final Resources resources = Resources.getResources(null);
        final LogRecord record = (param!=null) ? resources.getLogRecord(Level.FINE, clé, param) : resources.getLogRecord(Level.FINE, clé);
