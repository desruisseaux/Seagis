...
public class DataBase
{
    ...
    /**
     * Nom de la base de données. Il s'agit du nom
     * <code>name</code> fourni au constructeur.
     */
     ...
     * les dates écrites dans la base de données.
     */
    ...
    public DataBase() throws SQLException
    {
        try
        {
            Class.forName(Table.preferences.get("Driver", Main.DRIVER));
        }
        catch (ClassNotFoundException exception)
        {
            final LogRecord warning = new LogRecord(Level.WARNING, exception.getLocalizedMessage());
            warning.setThrown(exception);
            Table.logger.log(warning);
        }
        this.name       = Table.preferences.get("Source", Main.SOURCE);
     ...
     *
     * @param  name Protocole et nom de la base de données d'images.
     ...
     *         à la base de données.
     */
    public DataBase(final String name, final TimeZone timezone) throws SQLException
    {
        this.name       = name;
        this.connection = DriverManager.getConnection(name);
     ...
     *         à la base de données.
     */
    public DataBase(final String name, final TimeZone timezone, final String user, final String password) throws SQLException
    {
        this.name       = name;
        this.connection = DriverManager.getConnection(name, user, password);
        this.timezone   = timezone;
    }
    ...
    public synchronized ImageTable getImageTable() throws SQLException
    {
        if (series==null)
        {
            ...
            final SeriesEntry[] entries = table.getSeries();
            for (int i=entries.length; --i>=0;)
            {
                final int imageCount = table.getImageCount(entries[i]);
    ...
    private static SQLEditor getSQLEditor(final Preferences preferences)
    {
        ...
        final SQLEditor editor=new SQLEditor(preferences, Resources.format(Clé.EDIT_SQL_IMAGES_OR_FISHERIES¤1, new Integer(0)), Table.logger);
        for (int i=0; i<PROPERTY_NAMES.length; i++)
        {
            editor.addSQL(Resources.format(PROPERTY_NAMES[i]), DEFAULT_PROPERTIES[i*2+1], DEFAULT_PROPERTIES[i*2]);
        }
        return editor;
    }
     ...
     *
     * <blockquote><pre>
     * java fr.ird.image.sql.DataBase <var>options</var>
     * </pre></blockquote>
     *
