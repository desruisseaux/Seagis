...
final class SeriesTableImpl extends Table implements SeriesTable
{
     ...
     */
    public synchronized SeriesEntry[] getSeries() throws SQLException
    {
        ...
        final List<SeriesReference> list = new ArrayList<SeriesReference>();
        ...
        resultSet.close();
        statement.close();
        return list.toArray(new SeriesReference[list.size()]);
    }
     ...
     * La façon préférée d'obtenir des objets de cette classe est par la méthode {@link DataBase#getReference(int)}.
     *
     * @version 1.0
     * @author Martin Desruisseaux
     */
