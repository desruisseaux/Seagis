...
/**
 * Classe ayant la charge de compléter un "clip" fait sur des polygones.
 * Le constructeur reçoit en argument un rectangle {@link Rectangle2D},
 * qui représente le contour du "clip". La méthode {@link #getClipped}
...
final class Clipper {
    ...
    /**
     * Région à couper. Il s'agit des mêmes coordonnées que {@link #logicalClip},
     ...
     * à un usage interne par {@link #getClipped}, qui le construira et le modifiera
     * constament.
     */
    ...
    /**
     * Coordonnées des points d'intersections entre un polygone et {@link #clip}.
     * Ce tableau est réservé à un usage interne par {@link #getClipped}, qui le
     * construira et le modifiera constament.
     */
    ...
    /**
     * Projète le clip vers le système de coordonnées spécifié.
     *
     * @param  Système de coordonnées natif du polygone qui sera à couper.
     * @return Un rectangle représentant la région à couper. Ce rectangle
     ...
     *
     * @param clockwise Indique de quelle façon il faudra tourner autour du rectangle {@link #clip}
     *        pour ajouter des points. Une valeur positive tournera dans le sens des aiguilles d'une
     *        montre, tandis qu'une valeur négative tournera dans le sens inverse. La valeur 0 n'aura
     *        aucun effet.
     * @param x0 Coordonnées <var>x</var> du premier point détecté à l'extérieur du rectangle {@link #clip}.
     * @param y0 Coordonnées <var>y</var> du premier point détecté à l'extérieur du rectangle {@link #clip}.
    ...
    /**
     * Attache le polygone <code>sub</code> à la fin du polygone <code>result</code>.
    ...
    /**
     * Retourne un polygone qui ne contient que les points de <code>polygon</code> qui apparaissent dans
     * le rectangle spécifié au constructeur. Si aucun point du polygone n'appparaît à l'intérieur de
     * <code>clip</code>, alors cette méthode retourne <code>null</code>. Si tous les points du polygone
     * apparaissent à l'intérieur de <code>clip</code>, alors cette méthode retourne <code>polygon</code>.
     * Sinon, cette méthode retourne un polygone qui contiendra les points qui apparaissent à l'intérieur
     * de <code>clip</code>. Ces polygones partageront les mêmes données que <code>polygon</code> autant
     * que possible, de sorte que la consomation de mémoire devrait rester raisonable.
     * <br><br>
     * Note: avant d'appeller cette méthode, {@link #setCoordinateSystem} devrait avoir été
     *       appelée avec en argument le système de coordonnées natif de <code>polygon</code>.
     *
    ...
    public final Polygon getClipped(final Polygon polygon, final Segment.Iterator it) {
        ...
        intersectLength=0;
        Polygon result=null;
        final int polygonType=polygon.getInteriorSign();
        /*
        ...
        if (it.next(line)) {
            ...
            if (result != null) {
                 ...
                 */
                final float tmp[]=new float[borderLength];
