 ...
 *
 *   <li>Ils peuvent être chaînés avec d'autres objets <code>Segment</code>. Former une chaîne d'objets
 *       <code>Segment</code> peut être utile lorsque les coordonnées d'une côte ont été obtenues à partir
 *       de la digitalisation de plusieurs cartes bathymétriques, que l'on joindra en une ligne continue au
 *       moment du traçage. Elle peut aussi se produire lorsqu'une ligne qui se trouve près du bord de la
 *       carte entre, sort, réentre et resort plusieurs fois du cadre.</li>
 * </ul>
 *
...
final class Segment implements Serializable
{
     ...
     * Chaque objet <code>Segment</code> est capable d'accéder et d'agir sur les autres éléments de la liste
     * à laquelle il appartient.   En conséquent, il n'est pas nécessaire d'utiliser une classe séparée  (par
     * exemple {@link java.util.LinkedList}) comme conteneur.  Il ne s'agit pas forcément d'un bon concept de
    ...
    /**
     * Coordonnées à retourner après celles de <code>array</code>. Ces coordonnées servent généralement
     * à refermer un polygone, par exemple en suivant le cadre de la carte. Ce champ peut être nul s'il
     * ne s'applique pas.
     */
    ...
    /**
     * Construit des objets mémorisant les coordonnées <code>data</code>. Les valeurs <code>NaN</code>
     * au début et à la fin de <code>data</code> seront ignorées. Celles qui apparaissent au milieu
     * auront pour effet de séparer le trait en plusieurs segments.
     *
    ...
    /**
     * Construit des objets mémorisant les coordonnées <code>data</code> de l'index <code>lower</code>
     * inclusivement jusqu'à <code>upper</code> exclusivement. Ces index doivent se référer à la position
     * absolue dans le tableau <code>data</code>, c'est-à-dire être le double de l'index de la coordonnée.
     * Les valeurs <code>NaN</code> au début et à la fin de <code>data</code> seront ignorées. Celles qui
     * apparaissent au milieu auront pour effet de séparer le trait en plusieurs segments.
     *
     * @param data   Tableau de coordonnées (peut contenir des NaN).
    ...
    /**
     * Ajoute le segment <code>toAdd</code> à la fin du segment <code>queue</code>.      Les arguments
     * <code>queue</code> et <code>toAdd</code> peuvent être n'importe quel maillon d'une chaîne, mais
     * cette méthode sera plus rapide si <code>queue</code> est le dernier maillon.
     *
     * @param  queue <code>Segment</code> à la fin duquel ajouter <code>toAdd</code>. Si cet argument
     *               est nul, alors cette méthode retourne directement <code>toAdd</code>.
     * @param  toAdd <code>Segment</code> à ajouter à <code>queue</code>. Cet objet sera ajouté même s'il
     *               est vide. Si cet argument est nul, alors cette méthode retourne <code>queue</code>
     *               sans rien faire.
     ...
     * @throws IllegalArgumentException si <code>toAdd</code> avait déjà été ajouté à <code>queue</code>.
     */
     ...
     *               contiendra la première coordonnée, <code>points[1]</code> la seconde, etc. Si un
     *               élément de ce tableau est nul, un objet {@link Point2D} sera automatiquement créé.
     *
    ...
    {
        scan=getLast(scan);
        if (points.length==0) return; // Nécessaire pour l'implémentation ci-dessous.
        if (scan==null) throw new NoSuchElementException();
    ...
    {Utilities.unexpectedException("fr.ird.map", classe, method, exception);}
}
