...
public class AngleFormat extends Format
{
     ...
     * to {@link #applyPattern}. The argument <code>obj</code> shall be an {@link Angle} object
     * or one of its derived class ({@link Latitude}, {@link Longitude}). If <code>obj</code> is
     * a {@link Latitude} object, then a symbol "N" or "S" will be appended to the end of the string
     * (the symbol will be choosen according the angle's sign). Otherwise, if <code>obj</code> is a
     * {@link Longitude} object, then a symbol "E" or "W" will be appended to the end of the string.
     * Otherwise, no hemisphere symbol will be appended.
     * <br><br>
     * Strictly speaking, formatting ordinary numbers is not the <code>AngleFormat</code>'s job.
     * Nevertheless, this method accept {@link Number} objects. This capability is provided only
     ...
     *
     * @return The string buffer passed in as <code>toAppendTo</code>, with formatted text appended.
     * @throws IllegalArgumentException if <code>obj</code> if not an object of class {@link Angle} or {@link Number}.
     */
    ...
    /**
     * Parse a string as an angle. This method can parse an angle even if it doesn't comply
     * exactly to the expected pattern. For example, this method will parse correctly string
     * "<code>48°12.34'</code>" even if the expected pattern was "<code>DDMM.mm</code>" (i.e.
     * the string should have been "<code>4812.34</code>"). Spaces between degrees, minutes
     * and secondes are ignored. If the string ends with an hemisphere symbol "N" or "S",
     * then this method returns an object of class {@link Latitude}. Otherwise, if the string
     * ends with an hemisphere symbol "E" or "W", then this method returns an object of class
     * {@link Longitude}. Otherwise, this method returns an object of class {@link Angle}.
     *
