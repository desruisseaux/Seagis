        ...
        for (int j=0; j<byIndex.length; j++)
        {
            ...
            for (int i=j+1; i<byIndex.length; i++)
            {
                ...
                if (!(categ.lower>=check.upper || categ.upper<=check.lower)) // Do not accept NaN
                    throw new IllegalArgumentException(Resources.format(ResourceKeys.ERROR_RANGE_OVERLAP_$4,
...
------- END OF JDK 1.4 DEPENDENCIES ---*/
                ...
                for (int i=0; i<byIndex.length; i++)
                {
                    ...
                    Colors.expand(category.getColors(), ARGB, Math.round(category.lower), Math.round(category.upper));
                }
                model = Colors.getIndexColorModel(ARGB);
            }
        }
