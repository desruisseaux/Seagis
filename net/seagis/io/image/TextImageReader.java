...
public abstract class TextImageReader extends SimpleImageReader
{
    ...
    protected String getPositionString(final String message)
    {
        ...
        if (file!=null)
            if (line!=null)  position=resources.getString(ResourceKeys.FILE_POSITION_$2, file, line);
            else             position=resources.getString(ResourceKeys.FILE_$1,          file      );
        else if (line!=null) position=resources.getString(ResourceKeys.LINE_$1,                line);
        else                 position=null;
