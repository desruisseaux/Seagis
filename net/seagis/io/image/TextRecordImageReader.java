...
public class TextRecordImageReader extends TextImageReader
{
    ...
    /**
     * Données des images, ou <code>null</code> si aucune lecture n'a encore été faite. Chaque élément
     * contient les données de l'image à l'index correspondant    (i.e. l'élément <code>data[0]</code>
     * contient les données de l'image #0,    <code>data[1]</code> contient les données de l'image #1,
     * etc.).   Des éléments de ce tableau peuvent être nuls si les données des images correspondantes
     * ne sont pas retenues après chaque lecture (c'est-à-dire si <code>{@link #seekForwardOnly}==true</code>).
     */
    private RecordList[] data;

    /**
     * Index de la prochaine image à lire. Cet index n'est pas nécessairement égal à la
     * longueur du tableau {@link #data}. Il peut être aussi bien plus petit que plus grand.
     */
    ...
    /**
     * Retourne le numéro de colonne dans laquelle se trouvent les données de la bande spécifiée.
     * L'implémentation par défaut retourne <code>band</code> + 1 ou 2 si la bande est plus grand
     * ou égal à {@link #getColumnX} et/ou {@link #getColumnY}. Cette implémentation devrait
     * convenir pour des données se trouvant aussi bien avant qu'après les colonnes <var>x</var>
     ...
     * sont aussi acceptés, mais moins conseillés.
     */
    ...
    {processWarningOccurred(getPositionString(Resources.format(ResourceKeys.ERROR_BAD_COORDINATE_$1, new Float(coordinate))));}
    ...
    public static class Spi extends TextImageReader.Spi
    {
        ...
        public Spi(final String name, final String mime, final int xColumn, final int yColumn)
        {
            ...
            if (xColumn < 0) throw new IllegalArgumentException(Resources.format(ResourceKeys.ERROR_NEGATIVE_COLUMN_$2, "x", new Integer(xColumn)));
            if (yColumn < 0) throw new IllegalArgumentException(Resources.format(ResourceKeys.ERROR_NEGATIVE_COLUMN_$2, "y", new Integer(yColumn)));
            pluginClassName = "net.seagis.io.image.TextRecordImageReader";
        }
