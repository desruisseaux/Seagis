...
final class RecordList
{
    ...
    {if (data!=null) data=XArray.resize(data, upper);}
    ...
    private float getInterval(final int column, final float eps) throws IIOException
    {
        if (interval==null)
        {
            if (columnCount<=0) return Float.NaN;
        ...
        for (int i=1; i<count; i++)
        {
            ...
            {
                throw new IIOException(Resources.format(Clé.NOT_A_GRID));
            }
        }
    ...
    public String toString(final int xColumn, final int yColumn, final float eps)
    {
        ...
        buffer.append(Resources.format(Clé.POINT_COUNT¤3, new Integer(upper), new Float(xCount), new Float(yCount)));
