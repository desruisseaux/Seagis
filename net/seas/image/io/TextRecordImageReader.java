...
public class TextRecordImageReader extends TextImageReader
{
        ...
        if (imageIndex >= nextImageIndex)
        {
            ...
            for (;nextImageIndex<=imageIndex; nextImageIndex++)
            {
                ...
                if (records!=null)
                {
                    ...
                    if (lineCount<2)
                    {
                        throw new IIOException(getPositionString(Resources.format(Clé.FILE_HAS_TOO_FEW_DATA)));
                    }
                    if (data==null)
                    {
    ...
    {processWarningOccurred(getPositionString(Resources.format(Clé.BAD_COORDINATE¤1, new Float(coordinate))));}
    ...
    public static class Spi extends TextImageReader.Spi
    {
        ...
        public Spi(final String name, final String mime, final int xColumn, final int yColumn)
        {
            ...
            if (xColumn < 0) throw new IllegalArgumentException(Resources.format(Clé.NEGATIVE_COLUMN¤2, "x", new Integer(xColumn)));
            if (yColumn < 0) throw new IllegalArgumentException(Resources.format(Clé.NEGATIVE_COLUMN¤2, "y", new Integer(yColumn)));
