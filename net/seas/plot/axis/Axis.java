...
/**
 * An axis as a graduated line. <code>Axis</code> objets are really {@link Line2D}
 ...
 * point     ({@linkplain #getX1 <var>x<sub>1</sub></var>},{@linkplain #getY1 <var>y<sub>1</sub></var>}) to
 * end point ({@linkplain #getX2 <var>x<sub>2</sub></var>},{@linkplain #getY2 <var>y<sub>2</sub></var>}),
...
public class Axis extends Line2D implements Cloneable, Serializable
{
    ...
    /**
     * Police à utiliser pour l'écriture de la gradiation de l'axe.
     * Si ce champs est <code>null</code>, une police par défaut
     * sera utilisée.
     */
    private Font font;

    /**
     * Police à utiliser pour l'écriture de la légende de l'axe.
     * Si ce champs est <code>null</code>, une police par défaut
     * sera utilisée.
     */
    ...
    /**
     * Indique si cet axe peut afficher une légende. La valeur <code>true</code>
     * ne garanti pas qu'une légende sera effectivement écrite. Il faut aussi que
     * {@link #getLabel} retourne une chaîne non-nulle.
     */
    private boolean labelVisible;

    /**
     * Modèle qui contient les minimum,
     * maximum et la graduation de l'axe.
     */
     ...
     */
    public Axis(final Graduation graduation)
    {
    ...
    {
        final double dx = getX1() - getX2();
        final double dy = getY1() - getY2();
    ...
    /**
     * Sets the location of the endpoints of this <code>Axis</code> to the specified
     ...
     * @see #getY2
     */
    public synchronized void setLine(final double x1, final double y1, final double x2, final double y2) throws IllegalArgumentException
    {
    ...
    /**
     * Returns the font for axis graduations.
     */
    ...
    /**
     * Sets the font for axis graduations.
     *
     * @param font The new font, or <code>null</code>
     *             for reseting the default font.
     *
     * @see #getFont
     * @see #setLabelFont
     */
    ...
    /**
     * Returns the font for axis label.
     */
    ...
    /**
     * Sets the font for axis label.
     */
    public synchronized void setLabelFont(final Font labelFont)
    {
        modCount++; // Doit être en premier
        this.labelFont=labelFont;
    }

    /**
     * Tells whatever the axis's label is visible.
     */
    public boolean isLabelVisible()
    {return labelVisible;}

    /**
     * Sets the visible state of the axis's label.
     */
    public synchronized void setLabelVisible(final boolean visible)
    {
        this.labelVisible = visible;
        if (!visible) legendBounds=null;
    }
    ...
    public synchronized String getLabel()
    {
        ...
        final String label = graduation.getAxisLabel();
        final String  unit = graduation.getUnitLabel();
        if (unit!=null && unit.length()!=0)
        {
            return (label!=null) ? label+" ("+unit+')' : unit;
        }
        else return label;
    }
    ...
    /**
     * Returns an iterator object that iterates along the <code>Axis</code> boundary
    ...
    public synchronized java.awt.geom.PathIterator getPathIterator(final AffineTransform transform)
    {
        if (isPainting)
        {
            if (iterator!=null) iterator.rewind(transform);
            else iterator=new  TickPathIterator(transform);
            return iterator;
        }
        else return new PathIterator(transform, java.lang.Double.NaN);
    }
     ...
     * is specified, the coordinates returned in the iteration are transformed accordingly.
     */
    public synchronized java.awt.geom.PathIterator getPathIterator(final AffineTransform transform, final double flatness)
    {
        if (isPainting)
        {
            if (iterator!=null) iterator.rewind(transform);
        ...
        else return new PathIterator(transform, flatness);
    }
    ...
    public synchronized void paint(final Graphics2D graphics)
    {
         ...
         */
        if (iterator!=null)  iterator.init(null);
        else iterator=new TickPathIterator(null);
        ...
        if (!sameContext || labelBounds==null || clip.intersects(labelBounds) || maximumSize==null)
        {
            ...
            while (iterator.hasNext())
            {
                ...
                {
                    final GlyphVector glyphs = iterator.getLabelGlyphs();
                    final Rectangle2D bounds = iterator.getLabelBounds();
         ...
         */
        if (labelVisible && (!sameContext || legendBounds==null || clip.intersects(legendBounds)))
        {
     ...
     */
    public static AffineTransform createAffineTransform(final Axis xAxis, final Axis yAxis)
    {
        /*   x
         *  |
         *  |\
        ...
        synchronized (xAxis)
        {
            synchronized (yAxis)
            {
                ...
                ox = mx.getMinimum();
                oy = my.getMinimum();
                return new AffineTransform(exi, exj, eyi, eyj, xAxis.x1-(ox*exi+oy*eyi), yAxis.y1-(ox*exj+oy*eyj));
            }
        }
    }
     ...
     * access to the graduation values. Each <code>Axis.TickIterator</code>
     * object traverses the graduation of the underlying {@link Axis} object
     * independently from any other {@link TickIterator} objects in use at
     * the same time. If a change occurs in the underlying {@link Axis} object
     ...
     * @author Martin Desruisseaux
     */
    public class TickIterator implements net.seas.plot.axis.TickIterator
    {
        /**
         * The underyling tick iterator.
         */
        private net.seas.plot.axis.TickIterator iterator;
         ...
         * {@link Axis#createAffineTransform}.
         */
        private double scaleX, scaleY;

        /**
         * <code>(tickX, tickY)</code> is a unitary
        ...
        /**
         * Value returned by the last call to {@link #getLabel}.
        ...
        /**
         * Value of {@link Axis#modCount} when {@link #init} was
         * last invoked. This value is used in order to detect
         * changes to the underlying {@link Axis} during iteration.
         */
        private transient int modCount;

        /**
         * Construct an iterator. If an optional {@link RenderingHints}
         * is specified, tick locations are adjusted according values
         * for {@link Graduation#VISUAL_AXIS_LENGTH} and
         * {@link Graduation#VISUAL_TICK_SPACING} keys.
         *
         ...
         */
        public TickIterator(final Map hints, final FontRenderContext fontContext)
        {
         ...
         */
        public Point2D getPosition(final Point2D dest)
        {
            final double     position = getValue()-minimum;
            ...
            else return new Point2D.Float((float)x, (float)y);
        }
         ...
         */
        public Line2D getTick(final Line2D dest)
        {
            ...
            else return new Line2D.Float((float)x1, (float)y1, (float)x2, (float)y2);
        }
         ...
         * together with {@link #getLabelBounds} for labels rendering. <strong>Do not
         * change the returned {@link GlyphVector}</strong>,  since the glyphs vector
         ...
         */
        public GlyphVector getLabelGlyphs()
        {
            if (glyphs==null)
            {
                final String label=getLabel();
                if (label!=null)
                {
                    glyphs=getFont().createGlyphVector(getFontRenderContext(), label);
                }
            }
            return glyphs;
        }
         ...
         *
         * <pre>
         * {@link Axis.TickIterator} iterator = axis.{@link Axis#getTickIterator getTickIterator(null)};
         *
         * while (iterator.{@link #hasNext() hasNext()})
         * {
         *     {@link GlyphVector} glyphs = iterator.{@link Axis.TickIterator#getLabelGlyphs() getLabelGlyphs()};
         *     {@link Rectangle2D} bounds = iterator.{@link Axis.TickIterator#getLabelBounds() getLabelBounds()};
         *     graphics.drawGlyphVector(glyphs, (float)bounds.getMinX(), (float)bounds.getMaxY());
         *     iterator.{@link #next() next()};
         * }
         * </pre>
         *
         * This method returns <code>null</code> if it can't
         * compute bounding box for current tick.
         */
        public Rectangle2D getLabelBounds()
        {
            final GlyphVector glyphs = getLabelGlyphs();
            if (glyphs==null) return null;
            final Rectangle2D bounds = glyphs.getVisualBounds();
            final double      height = bounds.getHeight();
            final double      width  = bounds.getWidth();
            final double  tickStart  = (0.5*height)-Math.min(Axis.this.tickStart, 0);
            final double    position = getValue()-minimum;
            final double x= position*scaleX + getX1();
            final double y= position*scaleY + getY1();
         ...
         *
         * <pre>
         * {@link Graphics2D}           graphics = ...
         * {@link FontRenderContext} fontContext = graphics.getFontRenderContext();
         * {@link Font}                     font = axis.{@link Axis#getLabelFont() getLabelFont()};
         * {@link TickIterator}         iterator = axis.{@link Axis#getTickIterator getTickIterator(null)};
         *
         * {@link String}       label = axis.{@link Axis#getLabel() getLabel()};
         * {@link GlyphVector} glyphs = font.createGlyphVector(fontContext, label);
         * {@link Rectangle2D} bounds = centerAxisLabek(glyphs.getVisualBounds());
         * graphics.drawGlyphVector(glyphs, (float)bounds.getMinX(), (float)bounds.getMaxY());
         * </pre>
         *
         * @param  bounds  Un rectangle englobant les caractères à écrire. La position (<var>x</var>,<var>y</var>)
         *                 de ce rectangle est généralement (mais pas obligatoirement) l'origine (0,0). Ce rectangle
         *                 est habituellement obtenu par un appel à {@link Font#createGlyphVector(FontContext,String)}.
         * @param toRotate Si non-nul, transformation affine sur laquelle appliquer une rotation égale à l'angle de l'axe.
         *                 Cette méthode peut limiter la rotation aux quadrants 1 et 2 afin de conserver une lecture
         *                 agréable du texte.
         * @param maximumSize Largeur et hauteur maximales des étiquettes de graduation. Cette information est utilisée
         *                 pour écarter l'étiquette de l'axe suffisament pour qu'elle n'écrase pas les étiquettes de
         *                 graduation.
         * @return Le rectangle <code>bounds</code>, modifié pour être centré sur l'axe.
         */
        final Rectangle2D centerAxisLabel(final Rectangle2D bounds, final AffineTransform toRotate, final Dimension2D maximumSize)
        {
            final double height = bounds.getHeight();
            final double width  = bounds.getWidth();
            final double tx = 0;
         ...
         * garbage-collection by constructing an <code>Axis.TickIterator</code>
        ...
        public void refresh()
        {
            synchronized (Axis.this)
            {
                ...
                this.modCount = Axis.this.modCount;
            }
        }
        ...
        /**
         * Vérifie que l'axe n'a pas changé depuis le dernier appel de {@link #init}. Cette méthode doit
         * être appelée <u>à la fin</u> des méthodes de cette classe qui lisent les champs de {@link Axis}.
         */
        final void ensureValid()
        {if (this.modCount!=Axis.this.modCount) throw new ConcurrentModificationException();}
    }
    ...
    private class TickPathIterator extends TickIterator implements java.awt.geom.PathIterator
    {
         ...
         * la transformation affine créée par {@link Axis#createAffineTransform}.
         */
         ...
         *            de la transformation affine créée par {@link Axis#createAffineTransform}.
         */
    ...
    private final class PathIterator extends TickPathIterator
    {
        ...
        {return java.lang.Double.isNaN(flatness) ? shape.getPathIterator(transform) : shape.getPathIterator(transform, flatness);}
        ...
        public void next()
        {
            ...
            if (!isDone)
            {
                ...
                if (isDone())
                {
                     ...
                     */
                    isDone=true;
                    final String label=Axis.this.getLabel();
                    if (label!=null)
                    {
                        final GlyphVector glyphs=getLabelFont().createGlyphVector(getFontRenderContext(), label);
                        transform = (transform!=null) ? (AffineTransform) transform.clone() : new AffineTransform();
                        final Rectangle2D bounds=centerAxisLabel(glyphs.getVisualBounds(), transform, new XDimension2D.Double(maxWidth, maxHeight));
                        path=getPathIterator(glyphs.getOutline((float)bounds.getMinX(), (float)bounds.getMaxY()));
                    }
                }
            }
        }
        ...
        protected void prepareLabel()
        {
            ...
            {
                final GlyphVector glyphs = getLabelGlyphs();
                final Rectangle2D bounds = getLabelBounds();
                if (glyphs!=null && bounds!=null)
                {
