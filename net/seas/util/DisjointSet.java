...
/**
 * A set which is disjoint from others <code>DisjointSet</code>s. Two sets are disjoint (or
 * <em>mutually exclusive</em) if their intersection is the empty set. Adding an element to
 * a <code>DisjointSet</code> remove it from any other mutually exclusive <code>DisjointSet</code>.
 * Optionnaly, <code>DisjointSet</code>s may also have a trash set receiving removed elements.
 * The example below creates 3 mutually exclusive sets with a trash:
 *
 * <blockquote><pre>
 * DisjointSet set0 = new DisjointSet(true);
...
{
    // TODO: Can't generalize because of compiler bugs.
     ...
     *   <li>Otherwise, puts the key-value pair with the <code>trash</code> value in the underlying
     ...
     *       have a trash (<code>trash==null</code>), then the element is effectively removed from
     *       the underlying map.</li>
     * </ul>
     */
    private final Map<Object,DisjointSet> map;
     ...
     * set as argument.
     * <br><br>
     * Set constructed using this constructor has no trash. All remove operations
     * on this set really remove all references to the removed element, like a usual
     * {@link Set}. This is opposed to moving the element to a "trash" set, which is
     * allowed by the <code>DisjointSet(true)</code> constructor.
     */
    public DisjointSet()
    {this(false);}
    ...
    {
        map   = new LinkedHashMap<Object,DisjointSet>();
    ...
    public int size()
    {
        ...
        {
            int count=0;
            for (final Iterator<DisjointSet> it=map.values().iterator(); it.hasNext();)
            {
                if (it.next()==this) count++;
            }
            return count;
        }
    }
     ...
     */
    public boolean contains(final /*Element*/Object element)
    {
        synchronized (map)
     ...
     */
    public boolean add(final /*Element*/Object element)
    {
        synchronized (map)
     ...
     * @return <code>true</code> if the set changed as a result of the call.
     */
    public boolean remove(final /*Element*/Object element)
    {
        synchronized (map)
        {
    ...
    private final class Iter implements Iterator
    {
         ...
         */
        private final Iterator<Map.Entry<Object,DisjointSet>> iterator;

        /**
         * Entry for the next element to return, or
         * <code>null</code> if there is no more element.
         */
        private Map.Entry<Object,DisjointSet> prefetch;
         ...
         * iterator.
         */
        private Map.Entry<Object,DisjointSet> toRemove;
        ...
        private Map.Entry<Object,DisjointSet> prefetch()
        {
            ...
            if (prefetch==null)
            {
                ...
                {
                    final Map.Entry<Object,DisjointSet> next = iterator.next();
