 ...
 * la méthode {@link #getArea}, qui retourne les coordonnées logiques de la région à tracer.
 * Par exemple un objet <code>ZoomPane</code> qui trace une carte dont les coordonnées
 * géographiques vont de 10° à 15°E et de 40° à 45°N pourrait redéfinir cette méthode comme suit:
 *
 * <blockquote><pre>
 ...
 * </pre></blockquote>
 *
 * Il faut aussi redéfinir la méthode {@link #paintComponent(Graphics2D)} pour tracer
 * une carte ou un graphique. Notez qu'après la ligne <code>graphics.transform({link #zoom})</code>,
 * par défaut l'axe des <var>y</var> pointera vers le haut comme le veut la convention en géométrie.
 *
 * <blockquote><pre>
 ...
 * </pre></blockquote>
 *
 * Par défaut, la transformation affine {@link #zoom} sera initialisée de façon à
 * ce que les coordonnées logiques retournées par {@link #getPreferredArea} couvrent l'ensemble de
 * la surface visible de <code>ZoomPane</code>. Il est possible d'indiquer à <code>ZoomPane</code>
 * qu'il doit laisser une marge libre autour de son contenu en spécifiant un cadre vide.
 *
 * <blockquote><pre>
 * &nbsp;setBorder(BorderFactory.createEmptyBorder(top, left, bottom, right));
 * </pre></blockquote>
 *
 * Quelle que soit l'action faite par l'utilisateur, toutes les commandes de zooms sont
 * traduites par des appels à la méthode {@link #transform}. Les classes dérivées peuvent redéfinir
 * cette méthode si elles veulent prendre des actions particulières pendant les zooms, par exemple
 * modifier les minimums et maximums des axes d'un graphique. Le tableau ci-dessous énumère les touches
 ...
 * sont identifiées les différentes actions qui gèrent les zooms. Par exemple pour obtenir
 * l'action qui effectue le zoom avant, on pourrait écrire
 * <code>{@link #getActionMap() getActionMap()}.get("ZoomIn")</code>.
 *
 * <p><strong>Note: Les objets {@link JScrollPane} ne conviennent pas pour ajouter des barres de
 * défilements à un objet <code>ZoomPane</code>.</strong> Utilisez plutôt la méthode {@link #createScrollPane}.
 * Encore une fois, tous les déplacements faits par l'usager sur les barres de défilements seront traduits par
 * des appels à {@link #transform}. Notez aussi que si {@link #setPreferredSize} n'a pas été appelée avec une
 * dimension non-nulle, alors par défaut {@link #getPreferredSize} retourne la taille (en pixels) qu'occuperait
 * la composante pour être affichée complètement avec le zoom {@link #zoom} courant.</p>
 *
 * @version 1.0
...
public abstract class ZoomPane extends JComponent
{
    ...
    /**
     * Constante désignant les changements d'échelles selon les deux axes <var>x</var>
     * et <var>y</var>, avec comme condition supplémentaires que ces changements doivent
     * être uniformes. Ce drapeau implique {@link #SCALE_X} et {@link #SCALE_Y}. L'inverse
     * toutefois (<code>{@link #SCALE_X}|{@link #SCALE_Y}</code>) n'implique pas
    ...
    /**
     * Constante désignant le rétablissement de l'échelle, rotation et translation à une
     * valeur par défaut qui fait apparaître la totalité du graphique dans la fenêtre.
     * Cette commande se traduira par un appel à la méthode {@link #reset}.
     */
    public static final int RESET = (1 << 6);

    /**
     * Constante désignant un zoom par défaut proche du zoom maximal permis. Ce zoom
     * doit permettre de voir les détails du graphique sans être exagérément gros.
     * Note: ce drapeau n'aura pas d'effet si au moins un des drapeaux {@link #SCALE_X}
     * et {@link #SCALE_Y} n'est pas aussi spécifié.
     */
    ...
    /**
     * Liste des clés qui identifiront les actions servant à faire des zooms. Ces
     * clés identifient aussi la ressources à utiliser pour faire apparaître la
    ...
    /**
     * Liste des codes des touches utilisées par défaut pour faire des zooms. Les éléments de ce
     * tableau vont par pairs. Les index pairs désignent le code de la touche du clavier, tandis
     * que les index impairs désignent le modificateur (CTRL ou SHIFT par exemple). Pour obtenir
     * l'objet {@link KeyStroke} pour une action numérotée <var>i</var>, on peut utiliser le code
     * suivant:
     *
     * <blockquote><pre>
    ...
    {
        public Dimension     getMinimumSize(final JComponent c) {return new Dimension(MINIMUM_SIZE,MINIMUM_SIZE);}
        public Dimension     getMaximumSize(final JComponent c) {return getPreferredSize(c);}
        public Dimension   getPreferredSize(final JComponent c) {return ((ZoomPane) c).getDefaultSize();}
    ...
    /**
     * Classe chargé d'être à l'écoute de différents évènements nécessaires au
     * bon fonctionnement de {@link ZoomPane}.  Cette classe surveillera les cliques de la souris
     * (pour eventuellement prendre le focus ou faire apparaître un menu contextuel), surveillera
     * les changements de taille de la composante (pour ajuster le zoom), etc.
     *
     * @version 1.0
     * @author Martin Desruisseaux
     */
    ...
    /**
     * Transformation affine contenant les facteurs de zooms, de translations et de rotations.
     * Lors du traçage d'une composante, cette transformation affine doit être combinée par un
     * appel à <code>{@link Graphics2D#transform(AffineTransform) Graphics2D.transform}(zoom)</code>.
     */
     ...
     * pour conserver la même région lorsque la taille ou la position de la composante change. Initialement,
     * ce rectangle est vide. Il ne deviendra non-vide si {@link #reset} est appelée et que {@link #getPreferredArea}
     * et {@link #getZoomableBounds} ont tout deux retourné des coordonnées valides.
     *
     * @see #getVisibleArea
     * @see #setVisibleArea
     */
    ...
    /**
     * Indique si cet objet <code>ZoomPane</code> doit être redessiné pendant que
     * l'utilisateur déplace le glissoir des barres de défilement. La valeur par
     * défaut est <code>false</code>, ce qui signifie que <code>ZoomPane</code>
    ...
    /**
     * Rectangle dans lequel placer les coordonnées retournées par {@link #getZoomableBounds}.
     * Cet objet est défini afin d'éviter d'allouer trop souvent des objets {@link Rectangle}.
     */
    private transient Rectangle cachedBounds;

    /**
     * Objet dans lequel enregistrer le résultat de {@link #getInsets}. Utilisé pour éviter
     * que {@link #getZoomableBounds} n'alloue trop souvent des objets {@link Insets}.
     */
    ...
    /**
     * Réinitialise la transformation affine {@link #zoom} de façon à annuler tout zoom, rotation et translation.
     * L'implémentation par défait initialise la transformation affine {@link #zoom} de façon à faire pointer l'axe
     * des <var>y</var> vers le haut et à faire apparaître dans le panneau l'ensemble de la région couverte par les
     * coordonnées logiques {@link #getPreferredArea}.
     * <br><br>
     * Note pour les classes dérivées: <code>reset()</code> est <u>la seule</u> méthode de <code>ZoomPane</code>
     * qui ne doit pas passer par {@link #transform(AffineTransform)} pour modifier le zoom. Cette exception est
     * nécessaire pour éviter de tomber dans une boucle sans fin.
     */
    public void reset()
    ...
    /**
     * Réinitialise la transformation affine {@link #zoom} de façon à annuler tout zoom, rotation et translation.
     * L'argument <code>yAxisUpward</code> indique si l'axe des <var>y</var> doit pointer vers le haut. La valeur
     * <code>false</code> le laisse pointer vers le bas. Cette méthode est offerte par commodité pour les classes
     * dérivées qui souhaite redéfinir {@link #reset()}.
     *
     * @param zoomableBounds Coordonnées en pixels de la région de l'écran dans laquelle dessiner. Cet
     *                       argument sera habituellement <code>{@link #getZoomableBounds(Rectangle)
     *                       getZoomableBounds}(null)</code>.
     * @param yAxisUpward    <code>true</code> s'il faut faire pointer l'axe des <var>y</var> vers le
    ...
    protected final void reset(final Rectangle zoomableBounds, final boolean yAxisUpward)
    {
        if (!zoomableBounds.isEmpty())
        {
            ...
            if (isValid(preferredArea))
            {
                ...
                getVisibleArea(zoomableBounds); // Force la mise à jour de 'visibleArea'.
                /*
    ...
    /**
     * Indique si les coordonnées logiques d'une région ont été définies. Cette méthode
     * retourne <code>true</code> si {@link #setPreferredArea} a été appelée avec un
     * argument non-nul.
     */
    ...
    /**
     * Retourne les coordonnées logiques de la région que l'on souhaite voir affichée
     * la première fois que <code>ZoomPane</code> apparaîtra à l'écran. Cette région
     * sera aussi affichée chaque fois qu'est appelée la méthode {link #reset}.
     * L'implémentation par défaut procède comme suit:
     *
     * <ul>
    ...
    /**
     * Retourne les coordonnées logiques de la région visible à l'écran. Dans le cas d'une carte géographique
     * par exemple, les coordonnées logiques peuvent être exprimées en degrés de latitudes/longitudes ou encore
     ...
     * la région spécifiée. Si {@link #zoom} contient une rotation, cette rotation
    ...
    private AffineTransform setVisibleArea(Rectangle2D source, Rectangle2D dest) throws IllegalArgumentException
    {
        ...
        /*
         * Convertit la destination en coordonnées logiques. On pourra ensuite appliquer un
         * zoom et une translation qui amenerait <code>source</code> dans <code>dest</code>.
         */
        try
        {
    ...
    /**
     * Effectue un zoom, une translation ou une rotation sur le contenue de <code>ZoomPane</code>.
     * Le type d'opération à effectuer dépend de l'argument <code>operation</code>:
     *
     * <ul>
     *   <li>{@link #TRANSLATE_X} effectue une translation le long de l'axe des <var>x</var>.
     *       L'argument <code>amount</code> spécifie la transformation à effectuer en nombre de
     *       pixels. Une valeur négative déplace vers la gauche tandis qu'une valeur positive
     *       déplace vers la droite.</li>
     *   <li>{@link #TRANSLATE_Y} effectue une translation le long de l'axe des <var>y</var>.
     *       L'argument <code>amount</code> spécifie la transformation à effectuer en nombre de
     *       pixels. Une valeur négative déplace vers le haut tandis qu'une valeur positive
     *       déplace vers le bas.</li>
     *   <li>{@link #UNIFORM_SCALE} effectue un zoom. L'argument <code>zoom</code> spécifie le zoom
     *       à effectuer. Une valeur supérieure à 1 effectuera un zoom avant, tandis qu'une valeur
     *       comprise entre 0 et 1 effectuera un zoom arrière.</li>
     *   <li>{@link #ROTATE} effectue une rotation. L'argument <code>zoom</code> spécifie l'angle
     *       de rotation en radians.</li>
     *   <li>{@link #RESET} Redéfinit le zoom à une échelle, rotation et translation par défaut.
     *       Cette opération aura pour effet de faire apparaître la totalité ou quasi-totalité
     *       du contenu de <code>ZoomPane</code>.</li>
     *   <li>{@link #DEFAULT_ZOOM} Effectue un zoom par défaut, proche du zoom maximal, qui fait
     *       voir les détails du contenu de <code>ZoomPane</code> mais sans les grossir
     *       exégarément.</li>
     * </ul>
     *
     * @param  operation Type d'opération à effectuer.
     * @param  amount Translation en pixels ({@link #TRANSLATE_X} et {@link #TRANSLATE_Y}),
     *         facteur d'échelle ({@link #SCALE_X} et {@link #SCALE_Y}) ou angle de rotation
     *         en radians ({@link #ROTATE}). Dans les autres cas, cet argument est ignoré et
     *         peut être {@link Double#NaN}.
     * @param  center Centre du zoom ({@link #SCALE_X} et {@link #SCALE_Y}) ou de la rotation
     *         (({@link #ROTATE}), en coordonnées pixels. La valeur <code>null</code> désigne
     *         une valeur par défaut, le plus souvent le centre de la fenêtre.
     * @throws UnsupportedOperationException si l'argument <code>operation</code> n'est pas reconnu.
     */
    private void transform(final int operation, final double amount, final Point2D center) throws UnsupportedOperationException
    {
        if ((operation & (RESET))!=0)
        {
            ...
            ////    RESET    ////
            /////////////////////
            if ((operation & ~(RESET))!=0) throw new UnsupportedOperationException();
            reset();
            return;
        }
        final AffineTransform change;
        ...
        if ((operation & (TRANSLATE_X|TRANSLATE_Y))!=0)
        {
            ...
            ////    TRANSLATE    ////
            /////////////////////////
            if ((operation & ~(TRANSLATE_X|TRANSLATE_Y))!=0) throw new UnsupportedOperationException();
        ...
        else
        {
            ...
            else
            {
                ...
                /*
                 * On accepte les largeurs et hauteurs de 0. Si toutefois le rectangle n'est
                 * pas valide (largeur ou hauteur négatif), alors on terminera cette méthode
                 * sans rien faire. Aucun zoom n'aura été effectué.
                 */
            }
            if ((operation & (ROTATE))!=0)
            {
                ...
                ////    ROTATE    ////
                //////////////////////
                if ((operation & ~(ROTATE))!=0) throw new UnsupportedOperationException();
            ...
            else if ((operation & (SCALE_X|SCALE_Y))!=0)
            {
                ...
                ////    SCALE    ////
                /////////////////////
                if ((operation & ~(UNIFORM_SCALE))!=0) throw new UnsupportedOperationException();
            ...
            else if ((operation & (DEFAULT_ZOOM))!=0)
            {
                ...
                ////    DEFAULT_ZOOM    ////
                ////////////////////////////
                if ((operation & ~(DEFAULT_ZOOM))!=0) throw new UnsupportedOperationException();
    ...
    public void addMouseListener(final MouseListener listener)
    {
        ...
        super.addMouseListener   (mouseSelectionTracker); // doit être le dernier!
    }
     ...
     *
     * @param change Transformation affine qui représente le changement dans le zoom.
     *               Soit <code>oldZoom</code> et <code>newZoom</code> les transformations
     *               affines de l'ancien et du nouveau zoom respectivement. Alors la relation
     *               <code>newZoom=oldZoom.{@link AffineTransform#concatenate concatenate}(change)</code>
     *               doit être respectée (aux erreurs d'arrondissements près). <strong>Notez que cette
     *               méthode peut modifier <code>change</code></strong> pour combiner en une seule
     *               transformation plusieurs appels consécutifs de <code>fireZoomChanged</code>.
     */
    ...
    /**
     * Préviens les classes dérivées que le zoom a changé. Contrairement à la méthode
     * {@link #fireZoomChanged} protégée, cette méthode privée ne modifie aucun champ
     * interne et n'essaye pas d'appeller d'autres méthodes de <code>ZoomPane</code>
     * comme {@link #getVisibleArea}. On évite ainsi une boucle sans fin lorsque cette
     * méthode est appelée par {@link #reset}.
     */
    ...
    /**
     * Méthode appelée automatiquement après que l'utilisateur ait sélectionnée une région à l'aide
     * de la souris. L'implémentation par défaut zoom la région <code>area</code> sélectionnée. Les
     * classes dérivées peuvent redéfinir cette méthode pour entreprendre une autre action.
     *
     * @param area Région sélectionnée par l'utilisateur, en coordonnées logiques.
     */
    protected void mouseSelectionPerformed(final Shape area)
    {
        ...
        if (isValid(rect)) setVisibleArea(rect);
    }
     ...
     *
     * La forme retournée sera généralement d'une classe dérivée de {@link RectangularShape},
     * mais peut aussi être de la classe {@link Line2D}. <strong>Tout autre classe risque de
     * déclancher une {@link ClassCastException} au moment de l'exécution</strong>.
     *
     * L'implémentation par défaut retourne toujours un objet {@link java.awt.geom.Rectangle2D}.
     *
     * @param  event Coordonnées logiques de la souris au moment ou le bouton a été enfoncé.
     *         Cette information peut être utilisée par les classes dérivées qui voudraient
     *         tenir compte de la position de la souris avant de choisir une forme géométrique.
     * @return Forme de la classe {link RectangularShape} ou {link Line2D}, ou <code>null</code>
     *         pour indiquer qu'on ne veut pas faire de sélection avec la souris.
     */
    ...
    /**
     * Spécifie si l'affichage de la loupe est autorisée sur cette composante. L'appel de cette
     * méthode avec la valeur <code>false</code> fera disparaître la loupe, supprimera le choix
     * "Afficher la loupe" du menu contextuel et fera ignorer tous les appels à
    ...
    public synchronized void setMagnifierEnabled(final boolean enabled)
    {
        ...
        if (!enabled) setMagnifierVisible(false);
    }

    /**
     * Corrige les coordonnées d'un pixel pour tenir compte de la présence de la loupe.
     * La point <code>point</code> doit contenir les coordonnées d'un pixel à l'écran.
     * Si la loupe est visible et que <code>point</code> se trouve sur cette loupe,
     * alors ses coordonnées seront corrigées pour faire comme si elle pointait sur
     * le même pixel, mais en l'absence de la loupe. En effet, la présence de la loupe
     * peut déplacer la position apparante des pixels.
     */
     ...
     * @param center  Coordonnée centrale à laquelle faire apparaître la loupe. Si la loupe était
     *                initialement invisible, elle apparaîtra centrée à cette coordonnée (ou au
     *                centre de l'écran si <code>center</code> est nul). Si la loupe était déjà
     *                visible et que <code>center</code> est non-nul, alors elle sera déplacée
     *                pour la centrer à la coordonnées spécifiée.
     */
    private synchronized void setMagnifierVisible(final boolean visible, final Point center)
    {
        if (visible && magnifierEnabled)
        {
            if (magnifier==null)
            {
                Rectangle bounds=getZoomableBounds(); // Ce rectangle provient d'une cache; ne pas modifier!
            ...
            else if (center!=null)
            {
                ...
                magnifier.setFrame(center.x-0.5*width, center.y-0.5*height, width, height);
            }
        }
    ...
    {
        if (!isValid(visibleArea) || zoomIsReset) reset();
        if (magnifier!=null) magnifier.setClip(getZoomableBounds());
        /*
    ...
    /**
     * Synchronise la position et l'étendu des models <var>x</var> et <var>y</var> avec la position
     * du zoom. Les models <var>x</var> et <var>y</var> sont généralement associés à des barres de
     * defilements horizontale et verticale. Lorsque la position d'une barre de défilement est ajustée,
     * le zomm sera ajusté en conséquence. Inversement, lorsque le zoom est modifié, les positions et
     * étendus des barres de défilements sont ajustées en conséquence.
     *
     * @param x Modèle de la barre de défilement horizontale, ou <code>null</code> s'il n'y en a pas.
     * @param y Modèle de la barre de défilement verticale, ou <code>null</code> s'il n'y en a pas.
     */
    ...
    /**
     * Annule la synchronisation entre les models <var>x</var> et <var>y</var> spécifiés et le zoom de
     * cet objet <code>ZoomPane</code>. Les objets {@link ChangeListener} et {@link ZoomChangeListener}
     * qui avait été créés seront supprimés.
     *
     * @param x Modèle de la barre de défilement horizontale, ou <code>null</code> s'il n'y en a pas.
     * @param y Modèle de la barre de défilement verticale, ou <code>null</code> s'il n'y en a pas.
     */
     ...
     * avec des barres de défilements. Bien que ce ne soit généralement pas utile, il serait
     * possible de synchroniser plusieurs paires d'objets {@link BoundedRangeModel} sur un
     * même objet <code>ZoomPane</code>.
     *
     * @author Martin Desruisseaux
     * @version 1.0
     */
     ...
     * <code>viewSize</code> les dimensions en pixels qu'aurait <code>ZoomPane</code> si
     * sa surface visible couvrait la totalité de la région {@link #getArea} avec le zoom
     * courant (Note: cette dimension <code>viewSize</code> peut être obtenues par {@link
     * #getPreferredSize} si {@link #setPreferredSize} n'a pas été appelée avec une valeur
     * non-nulle). Alors par définition la région {@link #getArea} convertit dans l'espace
     * des pixels donnerait le rectangle <code>bounds=Rectangle(0,&nbsp;0,&nbsp;,viewSize.width,&nbsp;,viewSize.height)</code>.
     * Cette méthode <code>scrollRectToVisible</code> permet de définir la sous-région de
     * <code>bounds</code> qui doit apparaître dans la fenêtre <code>ZoomPane</code>.
     */
    ...
    /**
     * Indique si cet objet <code>ZoomPane</code> doit être redessiné pendant que l'utilisateur
     * déplace le glissoir des barres de défilements. Les barres de défilements (ou autres models)
     * concernées sont celles qui ont été synchronisées avec cet objet <code>ZoomPane</code> à l'aide
     * de la méthode {@link #tieModels}. La valeur par défaut est <code>false</code>, ce qui signifie
     * que <code>ZoomPane</code> attendra que l'utilisateur ait relaché le glissoir avant de se redessiner.
     */
    public boolean isPaintingWhileAdjusting()
    {return paintingWhileAdjusting;}

    /**
     * Définit si cet objet <code>ZoomPane</code> devra redessiner la carte pendant que l'utilisateur
     * déplace le glissoir des barres de défilements. Il vaut mieux avoir un ordinateur assez rapide
     * pour donner la valeur <code>true</code> à ce drapeau.
     */
    public void setPaintingWhileAdjusting(final boolean flag)
    {paintingWhileAdjusting=flag;}

    /**
     * Déclare qu'une partie de ce paneau a besoin d'être redéssinée. Cette méthode
     * ne fait que redéfinir la méthode de la classe parente pour tenir compte du cas
     * où la loupe serait affichée.
     */
    ...
    /**
     * Déclare que la loupe a besoin d'être redéssinée. Une commande {@link #repaint()}
     * sera envoyée avec comme coordonnées les limites de la loupe (en tenant compte de
     * sa bordure).
     */
    ...
    {
        flag=IS_PRINTING;
        super.paintComponent(graphics); // Ne pas appeller 'super.printComponent' parce qu'on ne
                                        // veut pas qu'il appelle notre 'paintComponent' ci-haut.
    }

    /**
     * Retourne la dimension (en pixels) qu'aurait <code>ZoomPane</code> s'il affichait la totalité
     * de la région {@link #getArea} avec le zoom courant ({@link #zoom}). Cette méthode est pratique
     * pour déterminer les valeurs maximales à affecter aux barres de défilement. Par exemple la barre
     * horizontale pourrait couvrir la plage <code>[0..viewSize.width]</code> tandis que la barre
     * verticale pourrait couvrir la plage <code>[0..viewSize.height]</code>.
     */
    private final Dimension getViewSize()
    {
        if (!visibleArea.isEmpty()) // 'false' si la fenêtre n'a pas encore été dimensionnée.
        {
            Rectangle2D area=getArea();
            if (isValid(area))
            {
     ...
     * méthode peut être redéfinie si on veut effectuer les zooms sur une portion
     * du graphique plutôt que sur l'ensemble.
     */
    ...
    /**
     * Méthode appélée lorsqu'une exception inatendue est survenue. L'implémentation
     * par défaut enregistre la trace de l'exception dans le journal et réinitialise
     * le zoom.
     */
    ...
    /**
     * Vérifie si le rectangle <code>rect</code> est valide. Le rectangle sera considéré invalide
     * si sa largeur ou sa hauteur est inférieure ou égale à 0, ou si une de ses coordonnées est
     * infinie ou NaN.
     */
