...
/**
 * Contrôleur permettant à l'utilisateur de sélectionner une région sur une composante.
 * L'utilisateur doit cliquer sur un point de la composante, puis faire glisser le curseur
 * de la souris en tenant le bouton enfoncé. Pendant le glissement, la forme qui sera dessinée
 * sera généralement un rectangle. D'autres formes pourraient toutefois être utilisées, comme
 * par exemple une ellipse. Pour utiliser cette classe, il faut créer une classe dérivée qui
...
{
    /**
     * Rectangle pointillé représentant la région en train de se faire sélectionner par l'utilisateur.
     * Ce rectangle peut être vide. Ces coordonnées ne prendront une signification qu'entre les moments
     * où l'utilisateur a appuyé sur le bouton de la souris et le moment où il l'a relâché pour délimiter
     * une région. Par convention, la valeur <code>null</code> indique qu'une ligne doit être utilisée au
     * lieu d'une forme rectangulaire. Les coordonnées sont toujours exprimées en pixels.
     */
    ...
    /**
     * Spécifie les couleurs à utiliser pour tracer le contour d'une boîte lorsque
     * l'utilisateur sélectionne une région. Toutes les couleurs <code>a</code>
     * seront remplacées par les couleurs <code>b</code> et vis-versa.
     */
     ...
     *
     * @param  event Coordonnée de la souris au moment ou le bouton a été enfoncé.
     *         Cette information peut être utilisée par les classes dérivées qui
     *         voudraient tenir compte de la position de la souris avant de choisir
     *         une forme géométrique.
     * @return Forme de la classe {link RectangularShape} ou {link Line2D}, ou
     *         <code>null</code> pour indiquer qu'on ne veut pas faire de sélection.
     */
    ...
    /**
     * Retourne la forme géométrique entourant la dernière région sélectionée par l'utilisateur. Une
     * transformation affine facultative peut être spécifiée pour convertir en coordonnées logiques
     * la région sélectionnée par l'utilisateur. La classe de la forme retournée dépend du model que
     * retourne {@link #getModel}:
     *
     * <ul>
     *   <li>Si le modèle est nul (ce qui signifie que cet objet <code>MouseSelectionTracker</code>
     *       ne fait que dessiner une ligne entre deux points), alors l'objet retourné sera de la
     *       classe {@link Line2D}.</li>
     *   <li>Si le modèle est non-nul, alors l'objet retourné peut être de la même classe (le plus
     *       souvent {@link java.awt.geom.Rectangle2D}). Il peut toutefois y avoir des situations
     *       ou l'objet retourné sera d'une autre classe, par exemple si la transformation affine
     *       <code>transform</code> effectue une rotation.</li>
     * </ul>
     *
     * @param  transform Transformation affine qui sert à convertir les coordonnées logiques
     *         en coordonnées pixels. Il s'agit généralement de la transformation affine qui
     *         est utilisée dans une méthode <code>paint(...)</code> pour dessiner des formes
     *         exprimées en coordonnées logiques.
     * @return Une forme géométrique  entourant la dernière région sélectionée par l'utilisateur,
     *         ou <code>null</code> si aucune sélection n'a encore été faite.
     * @throws NoninvertibleTransformException Si la transformation affine <code>transform</code> ne
    ...
    public Shape getSelectedArea(final AffineTransform transform) throws NoninvertibleTransformException
    {
        ...
        if (transform!=null && !transform.isIdentity())
        {
            ...
            else
            {
                ...
                else return transform.createInverse().createTransformedShape(shape);
            }
        }
        else return (shape!=null) ? (Shape) shape.clone() : new Line2D.Float(ox,oy,px,py);
    }

    /**
     * Indique si on peut transformer la forme <code>shape</code> en appellant simplement sa méthode
     * <code>shape.setFrame(...)</code> plutôt que d'utiliser l'artillerie lourde qu'est la méthode
    ...
    /**
     * Informe ce controleur que la souris vient de glisser avec le bouton enfoncé.
     * L'implémentation par défaut observe ce glissement pour déplacer un coin du
     * rectangle servant à sélectionner une région. L'autre coin reste fixé à
     * l'endroit où était la souris au moment ou le bouton a été enfoncé.
     */
