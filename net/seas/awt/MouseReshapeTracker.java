...
public class MouseReshapeTracker extends MouseInputAdapter implements Shape
{
     ...
     * la position du rectangle à l'aide de valeurs entrées dans les champs. Ce nombre
     * doit obligatoire être supérieur ou égal à 1.
     */
    ...
    /**
     * Dernière coordonnée <em>relatives</em> de la souris. Cette information est
     * exprimée en coordonnées logiques (d'après la transformation affine inverse
     * de {@link #getTransform}). Les coordonnées sont relative au coin
     * (<var>x</var>,<var>y</var>) du rectangle.
     */
    private transient double mouseDX, mouseDY;

    /**
     * Coordonnées <code>x</code>, <code>y</code>, <code>width</code> et <code>height</code> d'une
     * boîte qui englobe complètement {@link #rectangle}. Ces coordonnées doivent être exprimées
     * en <strong>pixels</strong>. Au besoin, la transformation affine {@link #getTransform} peut
     * être utilisée pour passer des coordonnées pixels aux coordonnées logiques et inversement.
     */
     ...
     * champ est souvent identique à {@link #adjustingSides}. Mais contrairement à
     * {@link #adjustingSides}, il désigne un bord de la forme {@link #logicalShape}
     * et non un bord de la forme en pixels apparaissant à l'écran. C'est différent
     ...
     *
     * @param shape Forme géométrique rectangulaire. Il n'est pas obligatoire que cette forme
     *        soit un rectangle. Il pourrait s'agir par exemple d'un cercle.  Les coordonnées
     *        de cette forme seront les coordonnées initiales de la visière.   Il s'agit de
     *        coordonnées logiques et non de pixels.  Notez que le constructeur retient une
     *        référence directe vers cette forme, sans faire de clone. En conséquent, toute
     *        modification faite à la forme géométrique se répercutera sur cet objet
     ...
     *   <li>{@link #x}, {@link #y}, {@link #width} et {@link #height} pour les coordonnées en pixels de {@link #drawnShape}.</li>
     * </ul>
     */
    private void update()
    {
        ...
        /*
         * NOTE: la condition 'drawnShape==logicalShape' indique qu'il n'a pas été nécessaire
         *       de modifier la forme. La méthode 'mouseDragged' utilisera cette information.
         *
         * Retient maintenant les coordonnées en pixels de la nouvelle position du rectangle.
         */
    ...
    /**
     * Déclare la transformation affine servant à transformer les coordonnées logiques en coordonnées
     * pixels. Il s'agit de la transformation affine spécifiée à {@link java.awt.Graphics2D#transform}
     * lors du dernier traçage de <code>this</code>. Les informations contenues dans cette transformation
     * affine sont nécessaires au fonctionnement de plusieurs méthodes de cette classe. Il est de la
     * responsabilité du programmeur de s'assurer que cette information soit toujours à jour. Par défaut,
    ...
    /**
     * Définit une nouvelle position et dimension pour le rectangle. Les coordonnées transmises
     * à cette méthode doivent être des coordonnées logiques plutôt que des pixels. Si la plage
     * des valeurs que peut couvrir le rectangle a été limitée par un appel à {@link #setClip},
     * alors le rectangle sera déplacé et au besoin redimensionné pour la faire entrer dans la
     * région permise.
     *
    ...
    /**
     * Définit la nouvelle plage de valeurs couverte par le rectangle selon l'axe
     * des <var>x</var>. Les valeurs couvertes le long de l'axe des <var>y</var>
     * ne seront pas changées. Les valeurs doivent être exprimées en coordonnées
     * logiques.
     *
     * @see #getMinX
     * @see #getMaxX
    ...
    /**
     * Définit les bornes entre lesquelles le rectangle peut se déplacer. Cette méthode se
     * contente d'appeller {@link #setClipMinMax setClipMinMax(...)} avec les paramètres
     * appropriés. Elle est définie afin de réduire les risques de confusion de la part des
     * programmeurs habitués aux conventions de <em>Java2D</em>. Si vous voulez spécifier
     * des valeurs infinies (pour étendre les limites de la visière à toutes les valeurs
    ...
    /**
     * Définit les bornes entre lesquelles le rectangle peut se déplacer. Notez que les arguments de
     ...
     * vers l'infinie. Une meilleure convention aurait été d'utiliser les valeurs minimales et maximales selon
     * <var>x</var> et <var>y</var>, ce que fait cette méthode.
     * <br><br>
     * Les arguments de cette méthodes définissent les valeurs minimales et maximales que peuvent prendre
     * les coordonnées logiques du rectangle. Les valeurs {@link java.lang.Double#NEGATIVE_INFINITY} et
     * {@link java.lang.Double#POSITIVE_INFINITY} sont valides pour indiquer que la visière peut balayer
     * toutes les valeurs selon certains axes. La valeur {@link java.lang.Double#NaN} pour un argument
     * donnée indique que l'on souhaite conserver l'ancienne valeur. Si la visière n'entre pas complètement
     * dans les nouvelles limites, elle sera déplacée et au besoin redimmensionnée de façon a y entrer.
     */
    ...
    /**
     * Méthode appellée automatiquement lorsqu'un changement du clip serait souhaitable.
     * Cette méthode peut être appellée par exemple lorsque l'utilisateur a édité manuellement
     * la position du rectangle dans un champ texte, et que la nouvelle position tombe en
     * dehors du clip actuel. Cette méthode n'est <u>pas</u> obligé d'accepter un changement
     * de clip. Elle peut ne rien faire, ce qui équivaut à refuser tout changement. Elle peut
     ...
     * certaines conditions aux changements. L'implémentation par défaut ne fait rien, ce
     * qui signifie qu'aucun changement automatique de clip ne sera autorisé.
     */
    ...
    /**
     * Indique si la taille du rectangle peut être modifié à partir du bord spécifié.
     * Le bord spécifié doit être une des constantes suivantes:
     *
     ...
     * </table>
     *
     * Ces constantes désignent le bord visible sur l'écran. Par exemple <code>NORTH</code>
     * désigne toujours le bord du haut sur l'écran. Toutefois, ça peut correspondre à un
     * autre bord de la forme logique <code>this</code> dépendemment de la transformation
     * affine qui avait été spécifiée lors du dernier appel à {@link #setTransform}.  Par
     * exemple <code>AffineTransform.getScaleInstance(+1,-1)</code> a pour effet d'inverser
     * de faire apparaître au "nord" les valeurs <var>y</var><sub>max</sub> plutôt que
    ...
    /**
     * Spécifie si la taille du rectangle peut être modifié à partir du bord spécifié.
     * Le bord spécifié doit être une des constantes suivantes:
     *
     * <table border align=center cellpadding=8 bgcolor=floralwhite>
     * <tr><td>{@link SwingConstants#NORTH_WEST}</td><td>{@link SwingConstants#NORTH}</td><td>{@link SwingConstants#NORTH_EAST}</td></tr>
    ...
    public void mouseDragged(final MouseEvent e)
    {
        if (isDraging)
        {
            ...
            if (source!=null) try
            {
                ...
                /*
                 * Les coordonnées (x0, y0, dx, dy) donne maintenant la nouvelle position
                 * et dimension du rectangle. Mais avant de faire le changement,  vérifie
                 * si un seul bord était en cours d'ajustement. Si oui, on annule les
                 * changements selon l'autre bord (sinon, l'utilisateur pourrait déplacer
                 * verticalement le rectangle en même temps qu'il ajuste son bord droit ou
                 * gauche, ce qui n'est pas très pratique...).
                 */
    ...
    /**
     * Méthode appelée automatiquement lorsque l'utilisateur a relâché le bouton de la souris.
     * L'implémentation par défaut appelle {@link #stateChanged} avec l'argument <code>false</code>,
     * afin d'informer les classes dérivées que les changements sont terminés.
     */
    ...
    /**
     * Méthode appelée automatiquement <strong>avant</strong> que la position ou la dimension
     * de la visière n'ait changée.  L'appel de <code>stateWillChange</code> est généralement
     * suivit d'un appel de {@link #stateChanged}, <u>sauf</u> si le changement prévu n'a
     * finalement pas eu lieu. Les classes dérivées peuvent redéfinir cette méthode pour prendre
     * les actions nécessaires lorsqu'un changement est sur le point d'être effectué. Elles ne
     * devraient toutefois appeler aucune méthode qui risque de modifier l'état de cet objet.
    ...
    /**
     * Méthode appelée automatiquement <strong>après</strong> que la position ou la dimension de
     * la visière ait changée. L'appel de <code>stateChanged</code> a obligatoirement été précédé
     * d'un appel à {@link #stateWillChange}. Les classes dérivées peuvent redéfinir cette méthode
     * pour prendre les actions nécessaires lorsqu'un changement vient d'être effectué. Elles ne
     * devraient toutefois appeler aucune méthode qui risque de modifier l'état de cet objet.
     * L'implémentation par défaut ne fait rien.
     *
     * @param isAdjusting <code>true</code> si l'utilisateur
     ...
     * @param side      Bord du rectangle dont l'éditeur contrôle les coordonnées.
    ...
    private final class Control implements PropertyChangeListener
    {
        ...
        /**
         * Côté du rectangle à contrôler. Ce champ désigne le bord visible sur l'écran. Par
         * exemple <code>NORTH</code> désigne toujours le bord du haut sur l'écran. Toutefois,
         * ça peut correspondre à un autre bord de la forme logique {@link MouseReshapeTracker}
         * dépendemment de la transformation affine qui avait été spécifiée lors du dernier appel
         * à {@link MouseReshapeTracker#setTransform}. Par exemple <code>AffineTransform.getScaleInstance(+1,-1)</code>
         * a pour effet d'inverser de faire apparaître au "nord" les valeurs <var>y</var><sub>max</sub>
         ...
         *        correspondre à un autre bord de la forme logique {@link MouseReshapeTracker} dépendemment de
         *        la transformation affine qui avait été spécifiée lors du dernier appel à {@link MouseReshapeTracker#setTransform}.
         *        Par exemple <code>AffineTransform.getScaleInstance(+1,-1)</code> a pour effet d'inverser de faire
         *        apparaître au "nord" les valeurs <var>y</var><sub>max</sub> plutôt que <var>y</var><sub>min</sub>.
